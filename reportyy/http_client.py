...
from reportyy import error, six, util
...
try:
    from reportyy.six.moves import urllib
except ImportError:
...
try:
    ...
    try:
        ...
        if (major, minor, patch) < (0, 8, 8):
            sys.stderr.write(
                "Warning: the Reportyy library requires that your Python "
                '"requests" library be newer than version 0.8.8, but your '
                '"requests" library is version %s. Reportyy will fall back to '
...
from reportyy.six.moves.urllib.parse import urlparse
...
def new_default_http_client(*args, **kwargs):
    if urlfetch:
        ...
        if sys.version_info < (2, 7, 9):
            warnings.warn(
                "Warning: the Reportyy library is falling back to urllib2 "
...
class RequestsClient(HTTPClient):
        ...
        try:
            try:
                ...
                raise TypeError(
                    ...
                    '"requests" library is not compatible with Reportyy\'s '
    ...
    def _handle_request_error(self, e):
        ...
        if isinstance(e, requests.exceptions.SSLError):
            msg = (
                "Could not verify Reportyy's SSL certificate.  Please make "
        ...
        # Retry only timeout and connect errors; similar to urllib3 Retry
            ...
            msg = "Unexpected error communicating with Reportyy."
            err = "%s: %s" % (type(e).__name__, str(e))
        # Catch remaining request exceptions
            ...
            msg = (
                "Unexpected error communicating with Reportyy. "
...
class UrlFetchClient(HTTPClient):
    ...
    def __init__(self, verify_ssl_certs=True, proxy=None, deadline=55):
        ...
        if proxy:
            raise ValueError(
                ...
                "Set reportyy.default_http_client to either RequestsClient, "
    ...
    def _handle_request_error(self, e, url):
        if isinstance(e, urlfetch.InvalidURLError):
            msg = (
                "The Reportyy library attempted to fetch an "
                "invalid URL (%r). This is likely due to a bug "
                "in the Reportyy's Python bindings. " % (url,)
            )
        elif isinstance(e, urlfetch.DownloadError):
            msg = "There was a problem retrieving data from Reportyy."
        elif isinstance(e, urlfetch.ResponseTooLargeError):
            msg = (
                "There was a problem receiving all of your data from "
                "Reportyy.  This is likely due to a bug in Reportyy. "
            )
        else:
            msg = "Unexpected error communicating with Reportyy."
...
class PycurlClient(HTTPClient):
    ...
    def _handle_request_error(self, e):
        if e.args[0] in [
            ...
            msg = (
                "Could not connect to Reportyy.  Please check your "
